...
function App() {
  ...
  return (
    <div className="flex h-[560px] w-[440px] flex-col bg-slate-50 dark:bg-slate-950">
      <header className="border-b border-slate-200 bg-white px-3 py-2.5 dark:border-slate-800 dark:bg-slate-900">
        <h1 className="text-sm font-bold text-slate-900 dark:text-slate-100">
          webext-store — feature demo
        </h1>
        <p className="text-[11px] text-slate-500 dark:text-slate-400">
          Every API surface, exercised live against real extension storage. Also runs in the
          options page and as a badge on every tab (content script) — same package, three
          different contexts.
        </p>
      </header>

      <nav className="flex flex-wrap gap-1 border-b border-slate-200 bg-white px-2 py-1.5 dark:border-slate-800 dark:bg-slate-900">
        {TABS.map((tab) => (
          <button
            key={tab.id}
      ...
      <footer className="border-t border-slate-200 bg-white px-3 py-1.5 dark:border-slate-800 dark:bg-slate-900">
        <button
