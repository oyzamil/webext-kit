...
export function DefineItemPanel() {
	...
	const [settingsMeta, setSettingsMeta] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	useEffect(() => {
		...
		const unwatch = settingsItem.watch((newValue, oldValue) => {
			...
			log(
				`settingsItem.watch fired: ${JSON.stringify(oldValue)} -> ${JSON.stringify(newValue)}`,
			);
		});
		return unwatch;
	}, []);
	...
	const handleRemoveValue = async () => {
		...
		log(
			"settingsItem.removeValue({ removeMeta: true }) — next getValue() returns the fallback",
		);
	};
	...
	const handleMigrate = async () => {
		...
		log(
			"settingsItem.migrate() — runs pending migrations (no-op if already current)",
		);
	};
		...
		>
			<div>
				...
				<JsonBlock
					value={{
						value: settings,
						fallback: settingsItem.fallback,
						meta: settingsMeta,
					}}
				/>
			</div>
			...
			<div>
				<p className="font-medium text-slate-600 text-xs dark:text-slate-300">
					installIdItem — key <code>local:installId</code>, set once via{" "}
					<code>init</code>
				</p>
