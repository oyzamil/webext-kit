...
export function SnapshotPanel() {
	...
	const handleSnapshot = async () => {
		...
		const data = await storage.snapshot(
			area,
			keys.length ? { excludeKeys: keys } : undefined,
		);
		setSnapshot(data);
		log(
			`snapshot('${area}'${keys.length ? `, { excludeKeys: [${keys.join(", ")}] }` : ""})`,
		);
	};
	...
	const handleRestore = async () => {
		...
		log(
			`restoreSnapshot('${area}', <snapshot>) — new keys since then are kept, not overwritten`,
		);
	};
	...
	const handleImportFile = async (file: File) => {
		try {
			...
			setSnapshot(parsed);
			log(
				`loaded "${file.name}" — click restoreSnapshot to write it into '${area}'`,
			);
		} catch {
