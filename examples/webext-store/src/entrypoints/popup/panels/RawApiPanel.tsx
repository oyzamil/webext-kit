...
import { Button, Card, Field, Input, JsonBlock, LogList, Row, Select, Textarea } from '@/components/ui';
...
export function RawApiPanel() {
  ...
  const toggleWatch = () => {
    ...
    unwatchRef.current = storage.watch(fullKey, (newValue, oldValue) => {
      setResult(newValue);
      log(`watch fired: ${JSON.stringify(oldValue)} -> ${JSON.stringify(newValue)}`);
    });
    setWatching(true);
  ...
  return (
    ...
    >
      ...
      <Field label="Value (JSON)">
        <Textarea rows={2} value={valueJson} onChange={(e) => setValueJson(e.target.value)} />
      </Field>
      <Row>
        ...
        <Button variant={watching ? 'danger' : 'secondary'} onClick={toggleWatch}>
