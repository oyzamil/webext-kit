...
import { appSettingItem, type AppSetting } from '@/utils/storage-items';
...
export function ObjectUpdatePanel() {
  ...
  const { value, loading, setValue, patchValue } = useStorage(appSettingItem, {
    onChange: (newValue, oldValue) => {
      ...
      const changedKeys = (Object.keys(after) as Array<keyof AppSetting>).filter(
        (k) => before[k] !== after[k],
      );
      log(
        changedKeys.length
  ...
  // clobber each other.
  const toggleTheme = () => patchValue({ theme: value.theme === 'dark' ? 'light' : 'dark' });
  ...
  return (
    ...
    >
      ...
      <div className="rounded-md bg-slate-900 p-2.5 text-[11px] leading-relaxed text-slate-100">
...
await setValue({ ...value, theme: 'light' });`}</pre>
        <p className="mt-2 font-semibold text-slate-300">Subscribing to changes:</p>
  ...
  onChange: (newValue, oldValue) => {
    ...
    if (newValue.theme !== oldValue?.theme) { /* ... */ }
        ...
        <p className="mt-2 text-slate-400">
          If two keys change independently and often, and you don't want every write to one to
          re-render/notify watchers of the other, give them separate items/keys instead of one
          object — that's the only way to get per-key granularity with a key-value store.
        </p>
      </div>
