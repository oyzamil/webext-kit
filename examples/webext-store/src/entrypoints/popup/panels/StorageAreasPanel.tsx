...
import { type StorageArea, type StorageItemKey, storage } from "webext-store";
...
const AREAS: Array<{ name: StorageArea; desc: string }> = [
	...
	{
		name: "session",
		desc: "In memory only — cleared when the browser/extension restarts.",
	},
	{
		name: "sync",
		desc: "Synced across the user's signed-in devices. Small quota (~100KB).",
	},
	{
		name: "managed",
		desc: "Read-only, set by enterprise policy. Writes here always fail.",
	},
];
...
export function StorageAreasPanel() {
	...
	return (
		<Card
			title="Storage areas"
			subtitle="Same API, four different persistence/sync behaviors"
		>
