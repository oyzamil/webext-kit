...
export const sendToContentScript: ExtMessaging.SendFx = async req => {
  const tabId = typeof req.tabId === 'number' ? req.tabId : (await getActiveTab())?.id
 ...
 */
export const relayMessage: ExtMessaging.MessageRelayFx = req => rawRelay(req, sendToBackground)
...
export const sendToBackgroundViaRelay: ExtMessaging.SendFx<MessageName> = rawSendViaRelay
