...
export type MessageName = [keyof MessagesMetadata] extends [never] ? string : keyof MessagesMetadata
export type PortName = [keyof PortsMetadata] extends [never] ? string : keyof PortsMetadata
...
export namespace ExtMessaging {
  ...
  export type Handler<RequestName = string, RequestBody = any, ResponseBody = any> = (
    request: Request<RequestName, RequestBody>,
    response: Response<ResponseBody>
  ) => void | Promise<void> | boolean

  export type PortHandler<RequestBody = any, ResponseBody = any> = Handler<PortName, RequestBody, ResponseBody>

  export type MessageHandler<RequestBody = any, ResponseBody = any> = Handler<MessageName, RequestBody, ResponseBody>

  export type SendFx<TName = string> = <RequestBody = any, ResponseBody = any>(
    request: Request<TName, RequestBody>,
    messagePort?: Pick<MessagePort, 'addEventListener' | 'removeEventListener' | 'postMessage'> | Window
  ) => Promise<ResponseBody>

  export type RelayFx = <RelayName = any, RequestBody = any, ResponseBody = any>(
    request: Request<RelayName, RequestBody>,
    onMessage?: (request: Request<RelayName, RequestBody>) => Promise<ResponseBody>,
    messagePort?: Pick<MessagePort, 'addEventListener' | 'removeEventListener' | 'postMessage'> | Window
  ) => () => void

  export type MessageRelayFx = <RequestBody = any>(request: Request<MessageName, RequestBody>) => () => void

  export type PortHook = <TRequestBody = Record<string, any>, TResponseBody = any>(
    name: PortName
  ) => {
    data?: TResponseBody
...
export type OriginContext = 'background' | 'extension-page' | 'sandbox-page' | 'content-script' | 'window'
