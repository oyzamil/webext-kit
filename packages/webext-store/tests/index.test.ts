...
describe("Storage Utils", () => {
	beforeEach(() => {
		...
		fakeBrowser.storage.local.set = vi.spyOn(
			fakeBrowser.storage.local,
			"set",
		) as unknown as typeof fakeBrowser.storage.local.set;
	});
	...
	describe.each(["local", "sync", "managed", "session"] as const)(
		...
		(storageArea) => {
			...
			describe("removeItems", () => {
				...
				it("should remove items using { item: StoreItem, options?: RemoveItemOptions } objects", async () => {
	...
	describe("defineItem", () => {
		describe("versioning", () => {
			it("should migrate values to the latest when a version upgrade is detected", async () => {
				...
				expect(migrateToV2).toHaveBeenCalledTimes(1);
				expect(migrateToV2).toHaveBeenCalledWith(2);

				expect(migrateToV3).toHaveBeenCalledTimes(1);
				expect(migrateToV3).toHaveBeenCalledWith(4);
			});
			...
			it("should set the version without running migrations for empty storage items", async () => {
				...
				expect(migrate).toHaveBeenCalledTimes(0);
			});
			...
			it("should call onMigrationComplete callback function if defined", async () => {
				...
				expect(onMigrationComplete).toHaveBeenCalledWith(12, 3);
			});
			...
			it("should skip missing migration functions", async () => {
				...
				expect(migrateToV1).toHaveBeenCalledTimes(1);
				expect(migrateToV1).toHaveBeenCalledWith(2);

				expect(migrateToV3).toHaveBeenCalledTimes(1);
				expect(migrateToV3).toHaveBeenCalledWith(2);
			});
			...
			it("should print migration logs if debug option is true", async () => {
				...
				expect(consoleSpy).toHaveBeenCalledWith(
					`[webext-store] Running storage migration for local:count: v1 -> v3`,
				);
				expect(consoleSpy).toHaveBeenCalledWith(
					`[webext-store] Storage migration processed for version: v2`,
				);
				expect(consoleSpy).toHaveBeenCalledWith(
					`[webext-store] Storage migration processed for version: v3`,
				);
				expect(consoleSpy).toHaveBeenCalledWith(
					`[webext-store] Storage migration completed for local:count v3`,
					{
			...
			describe("calling setValue", () => {
				...
				it("should set the version metadata when setting the value for the first time", async () => {
					...
					expect(fakeBrowser.storage.local.set).toHaveBeenCalledTimes(0);
					...
					expect(fakeBrowser.storage.local.set).toHaveBeenCalledTimes(2);
					...
					expect(fakeBrowser.storage.local.set).toHaveBeenCalledTimes(3);
					expect(fakeBrowser.storage.local.set).toHaveBeenCalledWith({
						count: 2,
					});
		...
		describe("types", () => {
			it("should define a nullable value when options are not passed", () => {
				...
				expectTypeOf(item2).toEqualTypeOf<StoreItem<number | null, {}>>();
			...
			it("should define a non-null value when options are passed with a nullish default value", () => {
				...
				expectTypeOf(item2).toEqualTypeOf<StoreItem<number, {}>>();
			});
	...
	describe("Multiple Storage Areas", () => {
		describe("getItems", () => {
			it("should get the values of multiple storage items efficiently", async () => {
				...
				expect(localGetSpy).toHaveBeenCalledWith(["item1", "item3"]);
				expect(sessionGetSpy).toHaveBeenCalledTimes(1);
				expect(sessionGetSpy).toHaveBeenCalledWith(["item2"]);
			});
		...
		describe("setItems", () => {
			it("should set the values of multiple storage items efficiently", async () => {
				...
				expect(sessionSetSpy).toHaveBeenCalledWith({ item2: value2 });
			});
		});
		...
		describe("setMetas", () => {
			it("should set metadata efficiently", async () => {
				...
				expect(localGetSpy).toHaveBeenCalledTimes(1);
				expect(localGetSpy).toHaveBeenCalledWith(["one$", "three$"]);
				expect(sessionGetSpy).toHaveBeenCalledTimes(1);
				expect(sessionGetSpy).toHaveBeenCalledWith(["two$"]);
